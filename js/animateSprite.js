(function ($, window, undefined) {
    ...
    var frame = function (frameNumber, manual) {
        ...
        return this.each(function () {
            if ($(this).data('animateSprite') !== undefined) {
                ...
                if (data.settings.verbose) {
                  data.settings.verboseListener(frameNumber);  //crossing fingers
                }

                if (manual != undefined)
                  console.log("Frame:" + frameNumber + ", column: " + column + ", width: " + data.settings.width + ", row: " + row + ", height: " + data.settings.height);
        ...
        return this.each(function () {
            ...
            if (typeof animationName === 'string') {
                ...
                if (data.settings.animations[animationName] !== data.currentAnimation) {
                  ...
                  // console.log("Running -> ",data.currentAnimation, data.currentFrame);
                  data.currentFrame = 0;
                }
    ...
    var addAnimation = function(name, frameset) {
      return this.each(function() {
        var $this = $(this),
          data = $this.data('animateSprite');

        data.settings.animations[name] = frameset;
      })
    }

    var verbose = function(listener, bool) {
      return this.each(function() {
        var $this = $(this),
          data = $this.data('animateSprite');

        if (listener == false) {
          data.settings.verboseListener = undefined;
          data.settings.verbose = false;
        } else {
          data.settings.verboseListener = listener;
          data.settings.verbose = bool;
        }
