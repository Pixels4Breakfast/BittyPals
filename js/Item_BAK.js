...
function BaseItem(ob) {
  var o = {
    ...
    get releaseDate() { return this._release_date; }
  }
  return o;
}
...
function Item(ob) {
  var o = {
    ...
    _con:$("#item" + ob.id), //container (jQuery friendly div container handle) [null is used to differentiate from 'unidentified']  ..should be generated by PHP
    _img:$("#item" + ob.id + "_img"), //image (jQuery friendly img tag handle) [null is used to differentiate from 'unidentified']  ..should be generated by PHP
    ...
    get name()      { return this._name; },
    get type()      { return this._type; },
    get isSprite()  { return this._isSprite; },
    get spriteInitialized() { return this._spriteInitialized; },
    ...
    get effectID()  { return this._effectID; },
    ...
    get z()         { return this._temp._z || this._z; },
    get x()         { return this._temp._x || this._x; },
    get y()         { return this._temp._y || this._y; },
    get rotate()    { return this._temp._r || this._r; },
    get m()         { return this._temp._m || this._m; },
    get scale()     { return this._temp._s || this._s; },
    get hr()        { return this._temp._hr || this._hr; },
    get hid()       { return this._hid; },
    get container() { return this._con; },
    get image()     { return this._img; /*return $("#item" + this._id + "_img");*/ },
    get hasPalette(){ return this._hasPalette; },
    get palette()   { return this._palette; },
    get data()      { return this._data; },

    //setters
    set spriteInitialized(v)  { this._spriteInitialized = v; },
    set fullInit(v)           { this._fullInit = v; },
    set baseId(v)             { this._baseId = v; },
    set src(v)                { this._src = v; },
    set name(v)               { console.error("cannot change baseSrc for " + this) },
    set type(v)               { console.error("cannot change type for " + this )},
    set id(v)                 { console.error("cannot change id for " + this) },
    set pid(v)                { console.error("cannot change pid for " + this) }, //trade will work around this
    set container(v)          { console.error("cannot change container for " + this )},
    set image(v)              { console.error("cannot change image for " + this )},
    set data(v)               { this._data = v; },
    set active(v)             { this._active = v; this._temp._active = v;
      //either add or remove item from habitat and send stuff through the database
    },
    set z(v)       { this._temp._z = v;
      this._con.css("z-index", v);
    },
    set x(v)       { this._temp._x = v;
      this._con.css("left", v + "px");
    },
    set y(v)       { this._temp._y = v;
      this._con.css("top", v + "px");
    },
    set rotate(v)  { this._temp._r = v;
    ...
    set scale(v)   { //setting the scale of the image and checking for mirroring
      ...
      this._img.css({
        ...
        '-webkit-transform': 'scaleY(' + scale + ') scaleX(' + scale*m + ')', // Chrome 4+, Op 15+, Saf 3.1, iOS Saf 3.2+
        '-moz-transform': 'scaleY(' + scale + ') scaleX(' + scale*m + ')', //FX 3.5-15
        '-ms-transform': 'scaleY(' + scale + ') scaleX(' + scale*m + ')', //IE 9
        '-o-transform': 'scaleY(' + scale + ') scaleX(' + scale*m + ')', //Op 10.5-12
        'transform': 'scaleY(' + scale + ') scaleX(' + scale*m + ')' //Fx 16+, IE10+
      });
    },
    set hr(v)      { this._temp._hr = v; //set the css hue-rotation of the _img
      this._img.css("filter", "hue-rotate("+v+"deg)");
      this._img.css({'-webkit-filter': 'hue-rotate('+v+'deg)'});
    },
    set hid(v)     { this._temp._hid = v;
    ...
    init: function(preloaded) {
      ...
      if (this.type != 'habitat' && this.type != 'effect') {
        this._palette = ($("#item" + ob.id + "_palette").attr("id") != undefined) ? $("#item" + ob.id + "_palette") : null;
        this.x = this._x;
        this.y = this._y;
        if (this._m) { this._temp._m = this._m; };
        ...
        if (preloaded) {
          this.z = this._z;
          zArray.push(this);
          this._con = $("#item" + this._id);
          this._img = $("#item" + this._id + "_img");
        ...
        } else { return this.toStage(); }
      }
    },
    toStage: function(pre) { //create the container and image div
      if ($("#roomController").hasClass("centerHV")) toggleRoomController();  //hide the controller if someone adds an item
      var preloaded = (pre != undefined && pre == true) ? true : false;
      // console.log(this._name + this._id + "::toStage");
      if (this.type == "habitat") {
        // console.log("swapping in habitat:");
        //$("#habitat").css('background-image', "url('"+this._src+"')");


        var background = $("#habBackground");
        background.empty();
        var img = $("<img/>", {src:this._src});
        img.css("z-index", "-1");
        this._img = img;
        ...
        console.log("Some days, it's just not worth chewing through the leather straps...");
        if (!preloaded) {
          this.hid = habitatID;

          this.commitTemp();
          paramQuery({update:'habitat', id:habitatID, values:{"inv_id":this.id, "item_id":this._baseId}}, validateSave);
        ...
        if ($("#fxLayer").attr('id') == undefined) {
          hab.append($("<div/>", { id:"fxLayer" }));
        }
        var fxLayer = $("#fxLayer");
        paramQuery({target:"fxLayer", effectID:this.effectID}, loadEffect, 'fetch_effect');
        updateInventory(this._baseId);
        this.hid = habitatID;
        this.commitTemp();
        ...
        if (this._con.attr("id") == undefined) {  //container does not exist in habitat, so create it
          ...
          var con = $("<div />", {id:"item" + this._id, class:"item_hab", item_id:this._id});

          if (this.isSprite) {
            // console.log("rendering sprite");
            var img = $("<div/>", {id:"item" + this._id + "_img", class:"item", style:"height:" + this.frameHeight + "px; width:" + this.frameWidth + "px;"});
          } else {
            var img = $("<img />", {id:"item" + this._id + "_img", class:"item", item_id:this._id});
          }
          ...
          tempImg.onload = function() {
            ...
            if (currentRoom != 1) {
              ...
              switch(habitatItem.roomDir) {
                case "hv":
                  if (currentRoom == 2 || currentRoom == 4) me.x += 1126;
                  if (currentRoom == 3 || currentRoom == 4) me.y += 430;
                break;
                case "v":
    ...
    toInventory: function(sort) {
      ...
      if (this.isSprite) if (this.fullInit) if (this.spriteInitialized) this.pause();
      this.fullInit = false;  this.spriteInitialized = false;

      var t = this._temp;
      t["_active"] = 0;  //make this stick...
      t["_hid"] = t["_x"] = t["_y"] = t["_z"] = t["_r"] = t["_hr"] = t["_m"] = 0; t["_s"] = 1;  //reset editing variables (in _temp)
      if (this.type == 'habitat') {
        this.commitTemp();
      } else if (this.type == 'effect') {
      ...
      if (inventoryObjects["item_" + this._baseId] != undefined) inventoryObjects["item_" + this._baseId].ids.push(this._id);
      updateInventory(this._baseId);
    },
    clearPalette: function() {
      if ($("#item" + this._id + "_palette").attr('id') != undefined) {
        $("#item" + this._id + "_palette").remove();
      }
    },
    createPalette: function() {
      ...
      if ($("#item" + this._id + "_palette").attr('id') != undefined) {
        this._palette = $("#item" + this._id + "_palette");
        return;
      }
      var itemPalette = $("<div />", {id:"item" + this._id + "_palette", class:"palette"});
      // console.log("appending: " + itemPalette.attr("id"));
      for (var i = 0; i == i; i++) {
        if ((HR_INCREMENT * i) > 359) break;
        if (pCSS) basePaletteStyle += ".p"+i+" { -webkit-filter : hue-rotate(" + (i*HR_INCREMENT) + "deg); -o-filter : hue-rotate(" + (i*HR_INCREMENT) + "deg); filter : hue-rotate(" + (i*HR_INCREMENT) + "deg); width:auto; height:25px; display:inline-block; margin-right:2px; margin-left:2px; }";  //this is in case there isn't already a style built out on the page

        if (this.isSprite) {
          var hAdjust = 25 / this.frameHeight;
          var wAdjust = 25 / this.frameWidth;
          var pScale = (hAdjust < wAdjust) ? hAdjust : wAdjust;
          var pStyle = "-webkit-transform:scale("+pScale+");-moz-transform:scale("+pScale+");-ms-transform:scale("+pScale+");-o-transform:scale("+pScale+");transform:scale("+pScale+");-webkit-transform-origin:top left;-moz-transform-origin:top left;-ms-transform-origin:top left;-o-transform-origin:top left;transform-origin:top left;";

          var thisP = $("<div/>", { style:"display:inline-block;margin-right:2px;margin-left:2px; overflow:hidden;height:"+this.frameHeight*pScale+"px; width:"+this.frameWidth*pScale+"px;"})
          thisP.append($("<div/>", {id:"p"+i, class:"p"+i, onclick:"rotatePalette(this);", style:"background-image:url("+this.src+"); height:"+this.frameHeight+"px; width:"+this.frameWidth+"px; overflow:hidden;display:inline-block; overflow:hidden; display:inline-block;position:relative;vertical-align:top;"+pStyle}));
          itemPalette.append(thisP);
        } else {
          itemPalette.append($("<img />", {id:"p" + i, src:this._src, class:"paletteItem p" + i, onclick:"rotatePalette(this);"}));
        }
    ...
    commitTemp: function() {
      ...
      if (arguments[0] != undefined) {  //commit single argument
        ...
        paramQuery({update:"inventory", id:this._id, values:changes}, validateSave);
      } else {  //commit all
        // console.error("commitTemp", this._temp);
        for(key in this._temp) {  //build the update string
          if (typeof this._temp[key] !== 'function') changes[key.split("_")[1]] = this._temp[key];  //remove the underscore so that we get the real DB field name
          if (key == "_s" && this._temp[key] == 0) return;
          this[key] = this._temp[key];  //set the object variable to match the temp value
        }
        if (Object.keys(this._temp).length == 0) return;
        console.error("\tCommitting (crimes against linear developers) -> ", changes);  //do something cheeky so that we know what the heck is going on
        paramQuery({update:"inventory", id:this._id, values:changes}, validateSave);
        delete this._temp;
        this._temp = {};
      }
    ...
    activate: function(firstLoad) {
      // console.log(this._id + " activated...");
      if (this.isSprite) {
        this.pause();
      }
      habitatItem.commitTemp();
      this._img.addClass("item_editing");
      this._img.on('mousedown touchstart', function(e){startDrag(e);});
    ...
    deactivate: function() {
      // console.log(this._id + " deactivated...");
      if (this.isSprite) {
        this.play();
      }
      saveChanges();
